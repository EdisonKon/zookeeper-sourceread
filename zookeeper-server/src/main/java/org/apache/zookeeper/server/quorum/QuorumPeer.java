...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    //加载database 并且比较一下zxid的epoch 和当前本机的epoch 还有接收到的epoch
    private void loadDataBase() {
        try {
            //与单机版一样,加载database 返回zxid 以便选举用
            zkDb.loadDataBase();

            // load the epochs
            //设置朝代和zxid
            long lastProcessedZxid = zkDb.getDataTree().lastProcessedZxid;
            //zxid的朝代是啥,最后处理完成的,有可能没写入到朝代文件
    ...
    synchronized public void startLeaderElection() {
        try {
            //判断当前服务的状态是不是正在找leader的状态
            if (getPeerState() == ServerState.LOOKING) {
                //创建一个投票 由 自己serverid database里的zxid 和 当前的epoch
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        //选举方式(也没有枚举..不知道啥意思,分析一下)
        switch (electionAlgorithm) {
        case 1:
            //非安全模式 AuthFastLeaderElection这个类在3.4.0里已经不推荐使用了
            le = new AuthFastLeaderElection(this);
            break;
        case 2:
            //安全模式 AuthFastLeaderElection这个类在3.4.0里已经不推荐使用了
            le = new AuthFastLeaderElection(this, true);
            break;
        case 3:
            //默认选举模式 快速选举 创建server间通信manager
            QuorumCnxManager qcm = createCnxnManager();
            //把这个管理器放到原子引用中,保证其他线程操作的时候同步可见
            QuorumCnxManager oldQcm = qcmRef.getAndSet(qcm);
            //存在旧的manager,那就把旧的manager停掉
            if (oldQcm != null) {
                LOG.warn("Clobbering already-set QuorumCnxManager (restarting leader election?)");
                //停止这个manager TODO 没进到里面去看
                oldQcm.halt();
            }
            //设置一个监听,消息的发送和接收 进入 Listener 查看
            QuorumCnxManager.Listener listener = qcm.listener;
            if(listener != null){
                listener.start();
                //快速选举(进入) 初始化快速选举对象,里面有提议的leader和zxid
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                //判断当前server的状态
                switch (getPeerState()) {
                case LOOKING: //正在寻找leader阶段
                    LOG.info("LOOKING");
                    ServerMetrics.LOOKING_COUNT.add(1);
                    //只读模式 有些机器宕机了 但是也必须需要提供可读的服务,所以才会有这种模式
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        try {
                           reconfigFlagClear();
                            ...
                            //设置当前的投票 *重要 (进入) makeLEStrategy是返回的选举算法
                            setCurrentVote(makeLEStrategy().lookForLeader());
                        } catch (Exception e) {
                            LOG.warn("Unexpected exception", e);
                            setPeerState(ServerState.LOOKING);
